...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_EDIT_TREESEED32790, &CMainFrame::OnEditTreeseed32790)
	ON_COMMAND(ID_EDIT_STARTANDHARVEST, &CMainFrame::OnEditStartandharvest)
END_MESSAGE_MAP()
...
/**
 * Changes the seed of the tree in the picture
 */
void CMainFrame::OnEditTreeseed32790()
{
	mPicture->GetTimeline()->GetTree()->ShowDialogueBox();

	mPicture->GetTimeline()->GetSecondTree()->ShowDialogueBox();
...
/**
 * Changes the start and harvest time of the tree in the pci
 */
void CMainFrame::OnEditStartandharvest()
{
	CStartAndHarvestDlg dlg;

	// Set the start and harvest times for the first tree
	dlg.SetTreeAdapter(mPicture->GetTimeline()->GetTree());
	if (dlg.DoModal() == IDOK)
	{
		dlg.Take();
		mPicture->UpdateObservers();
	...
	// Set the start and harvest times for the second tree
	CStartAndHarvestDlg secondDlg;
	secondDlg.SetTreeAdapter(mPicture->GetTimeline()->GetSecondTree());
	if (secondDlg.DoModal() == IDOK)
	{
		secondDlg.Take();
		mPicture->UpdateObservers();
	}
